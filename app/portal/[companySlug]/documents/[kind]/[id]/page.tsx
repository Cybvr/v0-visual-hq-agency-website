...
export default function PortalDocumentPage() {
  ...
  // Written documents sit under their own tab; the priced ones under billing.
  const backTab = kind === "document" ? "documents" : "billing"
  const backLabel = kind === "document" ? "Back to documents" : "Back to billing"
  if (!invoice && !contract && !estimate && !written) return <PortalNotice title="Document unavailable">This document hasn’t been issued to your company, or is no longer available. <Link href={`${portalPath(companySlug)}?tab=${backTab}`} className="underline">{backLabel}</Link></PortalNotice>
  const url = safeExternalUrl(invoice?.url || contract?.url) || undefined
  return <main className="mx-auto max-w-5xl px-5 py-8 sm:px-10"><div className="mb-7 flex flex-wrap items-center justify-between gap-4 print:hidden"><Link href={`${portalPath(companySlug)}?tab=${backTab}`} className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground"><ArrowLeft className="size-4" />{backLabel}</Link><DocumentActions url={url} title={invoice ? `Invoice ${invoice.invoiceNumber}` : contract?.title || estimate?.title || written?.title} /></div>
