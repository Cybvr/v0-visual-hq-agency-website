...
export default function PortalDocumentPage() {
  ...
  if (!invoice && !contract && !estimate) return <PortalNotice title="Document unavailable">This document hasn’t been issued to your company, or is no longer available. <Link href={`${portalPath(companySlug)}?tab=billing`} className="underline">Back to billing</Link></PortalNotice>
  const url = safeExternalUrl(invoice?.url || contract?.url) || undefined
  return <main className="mx-auto max-w-5xl px-5 py-8 sm:px-10"><div className="mb-7 flex flex-wrap items-center justify-between gap-4 print:hidden"><Link href={`${portalPath(companySlug)}?tab=billing`} className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground"><ArrowLeft className="size-4" />Back to billing</Link><DocumentActions url={url} title={invoice ? `Invoice ${invoice.invoiceNumber}` : contract?.title || estimate?.title} /></div>
