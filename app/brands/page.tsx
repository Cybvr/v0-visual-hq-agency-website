          ...
          <div className="mt-16 grid gap-6 md:grid-cols-2 lg:grid-cols-4">
            {items.map((item) => (
              <Link 
                key={item.slug} 
                href={item.href} 
                className="group relative flex flex-col justify-between rounded-[2rem] border border-border bg-card p-8 transition-all hover:border-primary/50 hover:shadow-2xl hover:shadow-primary/5"
              >
                <div>
                ...
                <ArrowUpRight className="mt-8 size-7 transition-transform group-hover:-translate-y-1 group-hover:translate-x-1 text-primary" />
              </Link>
            ))}
          </div>
        </div>
